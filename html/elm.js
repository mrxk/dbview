...
var author$project$Model$UpdateConnectionString = function (a) {
	return {$: 'UpdateConnectionString', a: a};
};
...
var author$project$Model$emptyQuery = {
...
var author$project$Main$init = function (_n0) {
	return _Utils_Tuple2(
		...
		elm$http$Http$get(
			{
				expect: elm$http$Http$expectString(author$project$Model$UpdateConnectionString),
										...
										return A3(
											...
											function (maybeRows) {
												if (maybeCount.$ === 'Just') {
													...
													var _n1 = _Utils_Tuple2(maybeColumns, maybeRows);
													if ((_n1.a.$ === 'Just') && (_n1.b.$ === 'Just')) {
														var columns = _n1.a.a;
														var rows = _n1.b.a;
														return elm$json$Json$Decode$succeed(
															elm$core$Result$Ok(
																{columns: columns, count: count, rows: rows}));
													} else {
														return elm$json$Json$Decode$succeed(
															elm$core$Result$Ok(
																{
													...
													var _n2 = _Utils_Tuple3(maybeMessage, maybeSeverity, maybeCode);
													if (((_n2.a.$ === 'Just') && (_n2.b.$ === 'Just')) && (_n2.c.$ === 'Just')) {
														var message = _n2.a.a;
														var severity = _n2.b.a;
														var code = _n2.c.a;
														return elm$json$Json$Decode$succeed(
															elm$core$Result$Err(
...
var author$project$Model$UpdateQueryResult = F2(
	function (a, b) {
		return {$: 'UpdateQueryResult', a: a, b: b};
	});
		...
		return elm$http$Http$post(
			{
				...
				expect: A2(
					...
					author$project$Model$UpdateQueryResult(id),
					author$project$Decoder$resultDecoder),
				url: '/postgres'
			});
	});
var author$project$Update$parseConnectionString = function (result) {
	if (result.$ === 'Err') {
		var error = result.a;
		switch (error.$) {
			case 'BadUrl':
...
var author$project$Update$parseErrorString = function (result) {
	if (result.$ === 'Err') {
		var error = result.a;
		switch (error.$) {
			case 'BadUrl':
	...
	function (msg, model) {
		switch (msg.$) {
			...
			case 'UpdateQueryResult':
				...
				var maybeQuery = A2(elm$core$Dict$get, id, model.queries);
				var errorString = author$project$Update$parseErrorString(httpResult);
				if (maybeQuery.$ === 'Just') {
					var query = maybeQuery.a;
					var newQueries = function () {
						if (httpResult.$ === 'Ok') {
							var queryResult = httpResult.a;
							return A3(
								elm$core$Dict$insert,
								id,
								_Utils_update(
								...
								_Utils_update(
									query,
									{
										result: elm$core$Result$Err(
											{code: '', message: errorString, severity: ''})
									}),
								model.queries);
						}
					}();
					var cmd = query.watch ? A2(
						elm$core$Task$perform,
						elm$core$Basics$always(
							author$project$Model$ExecuteQuery(id)),
						elm$core$Process$sleep(model.sleepTime)) : elm$core$Platform$Cmd$none;
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{queries: newQueries}),
			...
			case 'UpdateConnectionString':
				...
				var connectionString = author$project$Update$parseConnectionString(httpResult);
				return _Utils_Tuple2(
					_Utils_update(
						model,
			...
			case 'RemoveQuery':
				...
				if (maybeQuery.$ === 'Just') {
					var query = maybeQuery.a;
					var newQueries = A2(elm$core$Dict$remove, id, model.queries);
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{queries: newQueries}),
			...
			case 'AddQuery':
				var newQueries = A3(elm$core$Dict$insert, model.nextId, author$project$Model$emptyQuery, model.queries);
				var newNextId = model.nextId + 1;
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{nextId: newNextId, queries: newQueries}),
					elm$core$Platform$Cmd$none);
			case 'ExecuteQuery':
				var id = msg.a;
				var maybeQuery = A2(elm$core$Dict$get, id, model.queries);
				if (maybeQuery.$ === 'Just') {
					...
					return _Utils_Tuple2(
						model,
						A2(author$project$Update$doQuery, id, query.text));
				} else {
					return _Utils_Tuple2(model, elm$core$Platform$Cmd$none);
				}
			default:
				var timeString = msg.a;
				var newSleepTimeResult = elm$core$String$toFloat(timeString);
				if (newSleepTimeResult.$ === 'Just') {
					var newSleepTime = newSleepTimeResult.a;
					return _Utils_Tuple2(
						_Utils_update(
							model,
...
var author$project$View$viewQueryResultHeaderRow = function (headers) {
	return A2(
		elm$html$Html$tr,
		_List_Nil,
		A2(
...
var author$project$View$viewQueryResult = F2(
	function (model, result) {
		if (result.$ === 'Err') {
			...
			var _class = model.wrap ? 'wrap' : 'nowrap';
			return A2(
				elm$html$Html$div,
				_List_Nil,
				_List_fromArray(
					[
						...
						A2(
							elm$core$List$cons,
							author$project$View$viewQueryResultHeaderRow(success.columns),
							A2(elm$core$List$map, author$project$View$viewQueryResultDataRow, success.rows)))
					]));
		}
	});
...
var author$project$View$viewQuery = F3(
	function (model, id, query) {
		var htmlId = 'query' + elm$core$String$fromInt(id);
		return A2(
			elm$html$Html$div,
			_List_fromArray(
				[
					elm$html$Html$Attributes$id(htmlId),
					elm$html$Html$Attributes$class('queryContainer')
				]),
			_List_fromArray(
				[
					A2(
					...
					_List_fromArray(
						[
							elm$html$Html$Attributes$id('close_' + htmlId),
							elm$html$Html$Attributes$class('closeButton'),
							elm$html$Html$Events$onClick(
							author$project$Model$RemoveQuery(id))
						]),
					_List_fromArray(
						[
					...
					_List_fromArray(
						[
							elm$html$Html$Attributes$id('text_' + htmlId),
							elm$html$Html$Attributes$class('queryInput'),
							elm$html$Html$Attributes$value(query.text),
							elm$html$Html$Events$onInput(
							author$project$Model$UpdateQueryText(id))
						]),
					_List_Nil),
					A2(
					elm$html$Html$button,
					_List_fromArray(
						[
							elm$html$Html$Attributes$id('execute_' + htmlId),
							elm$html$Html$Attributes$class('executeButton'),
							elm$html$Html$Events$onClick(
							author$project$Model$ExecuteQuery(id))
						]),
					_List_fromArray(
					...
					_List_fromArray(
						[
							elm$html$Html$Attributes$id('watch_' + htmlId),
							elm$html$Html$Attributes$class('watchCheckbox'),
							elm$html$Html$Attributes$type_('checkbox'),
							elm$html$Html$Attributes$checked(query.watch),
							elm$html$Html$Events$onCheck(
							author$project$Model$UpdateQueryWatch(id))
						]),
					_List_Nil),
					A2(
					elm$html$Html$label,
					_List_fromArray(
						[
							elm$html$Html$Attributes$for('watch_' + htmlId),
							elm$html$Html$Attributes$class('watchLabel')
						]),
					_List_fromArray(
						[
					...
					_List_fromArray(
						[
							elm$html$Html$Attributes$id('output_' + htmlId),
							elm$html$Html$Attributes$class('output')
						]),
					_List_fromArray(
						[
							A2(author$project$View$viewQueryResult, model, query.result)
						]))
				]));
	});
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						elm$html$Html$text(model.connectionString)
					])),
				A2(
				elm$html$Html$div,
				_List_fromArray(
